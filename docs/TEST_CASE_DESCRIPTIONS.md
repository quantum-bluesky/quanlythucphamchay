...
| 176 | `IT-PUB-03` | Public ProductList xác định hàng có tồn kho nhỏ hơn đơn vị nhỏ nhất (theo bảng quy đổi hoặc đơn vị cơ sở) là hết hàng (hoặc sắp về nếu có hàng chờ nhập). |
...
| 69 | `IT-STS-01` | Kiểm tra các action đổi trạng thái, hủy, xóa phiếu và thao tác `Giá chung` ở đơn hàng/phiếu nhập đều hiện dialog confirm; nếu `Cancel` thì không đổi dữ liệu, còn `Confirm` mới áp dụng, đồng thời tồn kho hiển thị số mới ngay sau khi xuất kho không cần F5. |
...
| 144A | `IT-MOV-03` | Kiểm tra màn `Lịch sử biến động sản phẩm` mặc định hiển thị giao dịch mới hơn trước theo `Ngày giảm dần` và cho đổi sang `Ngày tăng dần` mà không làm sai list. |
| 144B | `IT-MOV-04` | Kiểm tra user có quyền điều chỉnh tồn nhìn thấy nút `Phiếu DC` ngay trên màn `Lịch sử biến động sản phẩm`, bấm vào sẽ quay lại màn tồn kho và mở sẵn phiếu điều chỉnh cho đúng sản phẩm đang xem. |

...
- Khi nội dung một file `.js` đổi trong cùng version chính, `N` của đúng file đó sẽ tự tăng thêm `1`.
- Khi `version` chính đổi, counter `N` của các file `.js` sẽ tự reset về `1`.

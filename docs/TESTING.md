...
- `Phase B UI`: tạo phiếu điều chỉnh trên màn tồn kho, tạo phiếu trả khách từ detail đơn cũ, tạo phiếu trả NCC từ detail phiếu nhập cũ, và không còn form trả hàng độc lập ngoài list
...
- `ACC-ORD-18`: UI `Xuất nhanh` phải cho user thường gửi request chờ duyệt, manager approve, rồi owner xử lý tiếp sau khi đã duyệt
...
- `UT-ORD-21`: store phải cập nhật lại đúng cart cũ khi card `Xuất nhanh` đã từng `Lưu nháp` hoặc đã `Chốt đơn`, không tạo cart mới và không làm tụt `committed -> draft`
...
- `IT-PUB-03`: Public ProductList phải xác định đúng trạng thái hết hàng khi tồn kho nhỏ hơn đơn vị nhỏ nhất (theo quy đổi hoặc cơ sở).

...
export const SCREEN_HELP = {
  ...
  "create-order": {
    ...
    steps: [
      ...
      "Ngay dưới card khách có thêm khối `Xử lý nhanh`: nhập khách, ngày xuất, thêm các dòng hàng và chọn `Đã xuất hàng` hoặc `Chỉ chốt đơn`; nếu tick `Đã thanh toán luôn`, app sẽ tự đi hết chuỗi `draft -> committed -> completed -> paid` chỉ với một lần lưu và vẫn ghi đủ audit/history.",
  ...
  purchases: {
    ...
    steps: [
      ...
      "Ngay dưới card NCC có thêm khối `Xử lý nhanh`: nhập NCC, ngày nhập, thêm các dòng hàng và chọn `Đã nhập hàng` hoặc `Chỉ đặt hàng`; nếu tick `Đã thanh toán luôn`, app sẽ tự đi hết chuỗi `draft -> ordered -> received -> paid` chỉ với một lần lưu và vẫn giữ đủ stock movement cùng audit/history.",

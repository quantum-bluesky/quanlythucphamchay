export function registerEntitiesControllerEvents(deps) {
  const {
    state,
    dom,
    renderCustomers,
  ...
  dom.openCartButton.addEventListener("click", () => {
    try {
      openCartForCustomer(dom.customerLookupInput.value);
    } catch (error) {
      showToast(error.message, true);
    }
  });
  ...
  dom.supplierList.addEventListener("click", (event) => {
    ...
    if (button.dataset.supplierAction === "use") {
      ...
      if (purchase) {
        updatePurchase(purchase.id, () => ({ supplierName: supplier.name, note: purchaseNoteInput.value.trim() }));
  ...
  dom.deletedProductList.addEventListener("click", async (event) => {
    ...
    const warning = [`Khôi phục sản phẩm ${product.name}?`, "Sản phẩm sẽ xuất hiện lại ở tồn kho, tạo đơn, nhập hàng và quản lý sản phẩm.", `Tồn hiện tại sau khi khôi phục: ${formatQuantity(product.current_stock)} ${product.unit}`].join("\n");
    if (!window.confirm(warning)) return;
    try {
      const data = await apiRequest(`/api/products/${productId}/restore`, { method: "POST", body: JSON.stringify({}) });
      await refreshData();
